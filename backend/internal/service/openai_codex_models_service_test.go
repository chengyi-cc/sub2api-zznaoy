...
func TestBuildCodexModelsManifestStripsGPTIdentityForNonGPTModels(t *testing.T) {
	...
	for _, m := range manifest.Models {
		...
		base := openai.CodexBaseInstructionsForModel(normalizeCodexModel(m.Slug))
		if strings.Contains(base, "GPT-") {
			require.True(t, strings.HasPrefix(tmpl, "You are Codex"), "%s: %.60q", m.Slug, tmpl)
		} else {
			// The pinned CLI fallback already has no GPT identity; preserve it verbatim.
			require.Equal(t, base, tmpl, m.Slug)
		}
