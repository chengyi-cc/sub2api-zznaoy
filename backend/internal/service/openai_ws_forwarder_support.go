...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if !account.IsOpenAIApiKey() && s.getOpenAIWSProtocolResolver().Resolve(account.forOpenAIModel(requestedModel)).Transport != OpenAIUpstreamTransportResponsesWebsocketV2 {
		return 0, nil, "", nil
	}
