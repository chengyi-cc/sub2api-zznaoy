...
// IsExcelBPSEnabled opts a direct ChatGPT OAuth account into the Excel gateway.
// It does not change credentials, scheduling or the saved native transport mode.
...
// 适用于 Anthropic OAuth/SetupToken 和 OpenAI OAuth 类型账号
// 启用后将模拟 Claude Code (Node.js) 客户端的 TLS 握手特征
func (a *Account) IsTLSFingerprintEnabled() bool {
	if a == nil || (!a.IsAnthropicOAuthOrSetupToken() && !(a.IsOpenAI() && a.Type == AccountTypeOAuth)) {
		return false
	}
	if a.Extra == nil {
		return false
