...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	if sub.NeedsDailyReset() {
		...
		if err := s.userSubRepo.ResetDailyUsage(ctx, sub.ID, expectedWindowStart, newStart); err != nil {
			return err
		}
	...
	if sub.NeedsWeeklyReset() {
		...
		if err := s.userSubRepo.ResetWeeklyUsage(ctx, sub.ID, expectedWindowStart, newStart); err != nil {
			return err
		}
	...
	if sub.NeedsMonthlyReset() {
		...
		if err := s.userSubRepo.ResetMonthlyUsage(ctx, sub.ID, expectedWindowStart, newStart); err != nil {
			return err
		}
