...
func (s *EmailService) SendEmailWithConfig(config *SMTPConfig, to, subject, body string) error {
	...
	return s.deliverSMTPMessage(config, message)
}
...
func (s *EmailService) SendEmailWithAttachmentWithConfig(config *SMTPConfig, to, subject, body string, attachment EmailAttachment) error {
	message, err := buildSMTPMessageWithAttachment(config, to, subject, body, attachment)
	if err != nil {
		return err
	}
	return s.deliverSMTPMessage(config, message)
...
// deliverSMTPMessage 投递已构造好的邮件；纯正文与带附件共用同一条连接/认证/投递路径，
// 保证附件邮件同样享有上游的隐式 TLS/STARTTLS 回退与超时控制。
func (s *EmailService) deliverSMTPMessage(config *SMTPConfig, message smtpMessage) error {
