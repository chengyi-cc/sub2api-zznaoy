	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rows := sqlmock.NewRows([]string{
				...
				"status_code", "upstream_status_code", "error_id", "phase", "severity", "message", "user_id", "api_key_id", "account_id", "group_id", "stream",
			}).
				AddRow("success", start, "req-slow", "openai", "gpt-5.5", 12000, 800, nil, nil, nil, nil, nil, nil, 1, 2, 3, 4, true).
				AddRow("error", start, "req-zero", "openai", "gpt-5.5", 9000, 0, 502, 429, 5, "upstream", "error", "failed", 1, 2, 3, 4, true).
			...
			require.Equal(t, 429, *items[1].UpstreamStatusCode)
