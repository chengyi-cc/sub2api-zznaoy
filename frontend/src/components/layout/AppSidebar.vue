...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/invoices', label: t('nav.invoices'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/redeem', label: t('nav.redeem'), icon: GiftIcon, hideInSimpleMode: true },
    { path: '/affiliate', label: t('nav.affiliate'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagAffiliate },
    { path: '/profile', label: t('nav.profile'), icon: UserIcon },
    ...customMenuItemsForUser.value.map((item): NavItem => ({
      path: item.open_mode === 'new_tab' ? item.url : `/custom/${item.id}`,
      label: item.label,
      icon: null,
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/invoices', label: t('nav.invoices'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagAdminPayment },
  ...
  if (authStore.isSimpleMode) {
    ...
    for (const cm of customMenuItemsForAdmin.value) {
      filtered.push({ path: cm.open_mode === 'new_tab' ? cm.url : `/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg, openMode: cm.open_mode, externalUrl: cm.url })
    }
    return filtered
  }
  ...
  for (const cm of customMenuItemsForAdmin.value) {
    visible.push({ path: cm.open_mode === 'new_tab' ? cm.url : `/custom/${cm.id}`, label: cm.label, icon: null, iconSvg: cm.icon_svg, openMode: cm.open_mode, externalUrl: cm.url })
  }
  return visible
})
