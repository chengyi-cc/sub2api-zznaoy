...
describe('TurnStateSettingsPanel', () => {
  it('saves model switches and custom refresh age and allows clearing exclusions', async () => {
    ...
    await flushPromises()
    expect(put).toHaveBeenLastCalledWith('/admin/accounts/turn-state/settings', expect.objectContaining({ refresh_after_minutes: 25, refresh_on_rejection: false, excluded_models: ['codex-auto-review', 'gpt-5.4'] }))
