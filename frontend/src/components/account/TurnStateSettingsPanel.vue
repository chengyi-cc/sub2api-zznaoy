    ...
    <fieldset v-if="settings" :disabled="saving" class="space-y-4">
      <div class="space-y-3">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <label class="text-xs">{{ label('候选国家（逗号分隔）', 'Candidate countries (comma separated)') }}<input v-model="settings.countries" class="input mt-1 w-full" placeholder="DE,GB,FR,SG,ZA,BR,AE,US,JP" data-testid="settings-countries"></label>
        </div>
