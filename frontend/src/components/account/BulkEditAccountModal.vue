        ...
        <fieldset id="bulk-edit-excel-bps-body" :disabled="!enableExcelBPS"
          ...
          <div v-if="excelBPSEnabled" class="mt-3 space-y-3">
            <div data-testid="bulk-excel-bps-model-selection">
              ...
              <p class="input-hint">{{ t('admin.accounts.openai.excelBPSModelsDesc') }}</p>
            </div>
            <label class="mt-3 flex cursor-pointer items-start gap-3">
              <input v-model="excelBPSAutoDisableOn403" type="checkbox" class="mt-0.5 h-4 w-4 rounded border-gray-300 text-primary-600" data-testid="bulk-excel-bps-auto-disable-on-403" />
              <span>
                <span class="block text-sm font-medium">{{ t('admin.accounts.openai.excelBPSAutoDisableOn403') }}</span>
                <span class="mt-1 block text-xs text-gray-500">{{ t('admin.accounts.openai.excelBPSAutoDisableOn403Desc') }}</span>
              </span>
            </label>
...
const buildUpdatePayload = (): Record<string, unknown> | null => {
  ...
  if (enableExcelBPS.value && allOpenAIOAuthOnly.value) {
    ...
    extra.openai_excel_bps_models = excelBPSEnabled.value
      ? [...new Set(excelBPSModels.value.map(model => model.trim()).filter(Boolean))]
      : null
    extra.openai_excel_bps_cache_creation_as_input = excelBPSEnabled.value && excelBPSCacheCreationAsInput.value
