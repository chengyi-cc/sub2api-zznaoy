export default {
    accounts: {
      accountTemplate: {
        ...
        applyNamed: 'Use {name}', noneEnabled: 'No enabled templates. Add or enable one in Edit template.', chooseTemplate: 'Choose a template to edit', addTemplate: 'Add template', removeTemplate: 'Delete template',
        collectionHint: 'Save up to 20 templates. Each enabled template gets its own apply button; disabled templates stay hidden. Switching preserves edits. Save applies all template changes; Cancel discards them, including deletions.',
        templateName: 'Template name', showButton: 'Enable and show apply button', templateEnabled: 'Enabled', templateDisabled: 'Disabled', unnamed: 'Unnamed', defaultName: 'Template {number}',
        invalidNames: 'Template names must be unique, nonempty and at most 64 characters, without line breaks or tabs.', noLongerEnabled: 'This template was disabled or deleted. Buttons have been refreshed; account settings are unchanged.',
      ...
      candyMonitor: {
        ...
        templateHint: 'Saved changes only affect accounts inheriting the template. Interval: 5–10080 minutes; retention: 10–500 results. Scheduled tests consume account usage.',
        ...
        group: 'Group', allGroups: 'All groups', platform: 'Platform', allPlatforms: 'All platforms', accountStatus: 'Account status', allStatuses: 'All statuses', statusActive: 'Active', statusInactive: 'Inactive', statusDisabled: 'Disabled', statusError: 'Error', monitorStatus: 'Monitor status', filter: 'Filter', reset: 'Reset', autoRefreshing: 'Refreshes every 10 seconds', account: 'Account', search: 'Search account name or ID', enabledOnly: 'Enabled only', refresh: 'Refresh',
        ...
        configuration: 'Schedule', latest: 'Latest status', historyStatus: 'Last 10', historyStatusHint: 'Green: 21 normal. Red: 29 or another abnormal answer. Amber: retest. Gray: no record.', lastTen: 'Old → New', otherShort: 'retest', nextRun: 'Next test', actions: 'Actions', enabled: 'Enabled', paused: 'Paused', inherited: 'Default template', custom: 'Custom', minutes: 'Every {count} min', globalPaused: 'Globally paused',
      ...
      openai: {
        ...
        excelBPSAutoDisableOn403Desc: 'Off by default. Disables only Excel for upstream HTTP 403, excluding model-access errors. Does not disable the account or replay the request.',
        testModeBPSTools: 'Excel tool roundtrip (up to 3 requests)',
        ...
        excelBPSNotice: 'Start a new Codex session after saving. Supports image URLs and inline uploads. Images are uploaded to OpenAI with this account and reuse file IDs; no image host or public fetch endpoint is needed. OpenAI retains the files; deletion after 5 minutes is not guaranteed. Upload failures return an error. Selected models use HTTP/SSE instead of WebSocket or passthrough; other models keep their saved transport settings. max / ultra effort is sent as xhigh; model access depends on the upstream account. Client parallel settings are honored. Requests declaring native hosted tools or other unsupported capabilities use the original route before dispatch.',
