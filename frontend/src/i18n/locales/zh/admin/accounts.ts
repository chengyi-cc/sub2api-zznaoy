export default {
    accounts: {
      ...
      candyMonitor: {
        ...
        templateHint: '保存后仅更新继承模板的账号。间隔 5–10080 分钟，保留 10–500 条记录；定时检测会消耗调用额度。',
        saveTemplate: '保存默认模板', scheduleHint: '只检测已启用的账号，每 30 秒检查一次到期任务。检测会消耗账号调用额度；关闭页面后仍继续。结果不会自动停用或恢复账号。',
        group: '分组', allGroups: '全部分组', platform: '平台', allPlatforms: '全部平台', accountStatus: '账号状态', allStatuses: '全部状态', statusActive: '正常', statusInactive: '停用', statusDisabled: '禁用', statusError: '错误', monitorStatus: '检测状态', filter: '筛选', reset: '重置', autoRefreshing: '每 10 秒刷新', account: '账号', search: '搜索账号名称或 ID', enabledOnly: '仅看已启用', refresh: '刷新',
        selected: '已选 {count} 个账号（当前页）', applyDefaults: '应用已保存模板并启用', pauseSelected: '暂停所选', selectPage: '选择当前页',
        configuration: '定时检测', latest: '最新状态', historyStatus: '最近 10 次', historyStatusHint: '绿色为 21 正常，红色为 29 或其他异常答案，黄色为待复测，灰色为暂无记录。', lastTen: '旧 → 新', otherShort: '待复测', nextRun: '下次检测', actions: '操作', enabled: '已启用', paused: '已暂停', inherited: '默认模板', custom: '单独设置', minutes: '每 {count} 分钟', globalPaused: '全局暂停',
      ...
      candy: {
        ...
        shape: '形状', apple: '苹果味', peach: '桃子味', melon: '西瓜味', round: '圆形', star: '五角星形',
        formatHint: '模型可以先分析，最后单独一行返回 CANDY_RESULT=数字，只给一个唯一的最终总数，不附带单位、条件或解释。后台只核对这一行，分析中提到的其它数字不参与判分。',
        disclaimer: '单题抽查不等于模型身份或整体智力鉴定。答错建议复测；格式不符、超时或回复被截断不会直接判定“降智”，也不会因答案错误禁用账号。',
        connected: '请求已完成，推理判分见糖果测试结果',
        sending: '正在发送糖果题，等待最终答案…',
        incomplete: '测试流已结束，但没有收到完成结果，请重试。',
        actual: '模型最终答案：{answer} 颗',
        expected: '标准答案：{answer} 颗',
        duration: '本次耗时 {seconds} 秒',
        verdict: { pass: '本题通过', incorrect: '答案未通过，建议复测', invalid_format: '答案格式不符，无法自动判分', inconclusive: '未获得完整有效结果，暂不判分' }
      },
      title: '账号管理',
      ...
      codexTimezoneRewriteDesc: '默认关闭。开启后按账号出口地址选择支持的时区，不可用时回退新加坡，并同步请求环境中的当前日期；不修改服务器时间。',
        ...
        excelBPSAutoDisableOn403Desc: '默认关闭。仅上游 HTTP 403 可关闭此账号的 Excel 开关；模型权限错误不触发，不停用账号、不重发当前请求。',
        testModeBPSTools: 'Excel 工具往返检测（最多 3 次请求）',
        ...
        excelBPSNotice: '保存后请新建 Codex 会话。支持图片链接和直接上传图片；上传图片通过当前账号发送到 OpenAI 附件接口，同图复用文件编号，无需图床或本站公网取图。图片由 OpenAI 保存，不保证 5 分钟后删除；上传失败会明确报错。勾选模型使用 HTTP 流式连接，不使用 WebSocket（双向长连接）及自动透传；未勾选模型保留原连接设置。max / ultra（推理强度）按 xhigh 发送，模型可用性以上游为准。 客户端并行设置会保留；请求声明原生联网搜索、文件工具等 Excel 不支持的能力时，发送前使用原通道。',
