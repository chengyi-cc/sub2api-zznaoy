export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSNotice: '保存后请新建 Codex 会话。支持图片链接和直接上传图片；上传图片临时保存在本机，同一会话复用固定链接，停止使用 5 分钟后失效，无需对象存储。站点地址需可通过 HTTPS 从外网访问。使用 HTTP 流式连接，不使用 WebSocket（双向长连接）及自动透传。max / ultra（推理强度）按 xhigh 发送，模型可用性以上游为准。',
