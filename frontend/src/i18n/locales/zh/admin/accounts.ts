export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSNotice: '保存后请新建 Codex 会话。支持图片链接；直接上传图片需先启用「备份 → 图片对象存储」，系统会自动生成 5 分钟有效的临时链接。使用 HTTP 流式连接，不使用 WebSocket（双向长连接）及自动透传。max / ultra（推理强度）按 xhigh 发送，模型可用性以上游为准。',
