...
describe('Ops request latency details', () => {
  ...
  it.each([true, false])('keeps client and upstream statuses alongside first-token latency (desktop: %s)', async (desktop) => {
    viewport.desktop = desktop
    listRequestDetails.mockResolvedValue({
      items: [{ kind: 'error', created_at: '2026-09-10T00:00:00Z', duration_ms: 12000, first_token_ms: 800, status_code: 502, upstream_status_code: 429 }],
      total: 1,
    })
    const wrapper = await openDetails('ttft_desc')
    expect(wrapper.text()).toContain('800 ms')
